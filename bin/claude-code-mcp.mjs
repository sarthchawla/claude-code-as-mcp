...
const tools = [
  tool("claude_run", "Run a Claude Code task. Set background to true to return a job ID immediately; use claude_status, claude_result, and claude_cancel to manage it.", {
    prompt: stringSchema("Task for Claude Code.", true), model: stringSchema("Optional per-invocation model override."), cwd: stringSchema("Working directory. Defaults to the server working directory."),
  ...
  tool("claude_review", "Run a read-only Claude Code review. Omit model to use the configured default.", {
    cwd: stringSchema("Directory to review."), model: stringSchema("Optional per-invocation model override."), prompt: stringSchema("Optional review focus."), timeout_ms: { type: "integer", minimum: 1, maximum: MAX_TIMEOUT_MS }
  }),
...
function validateArguments(name, args) {
  ...
  validateOptionalString(args, "model"); validateOptionalString(args, "cwd"); validateOptionalString(args, "prompt"); validateOptionalString(args, "review_model"); validateOptionalString(args, "job_id");
